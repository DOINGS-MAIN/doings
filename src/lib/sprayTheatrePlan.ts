...
export const DEFAULT_SPRAY_THEATRE_SETTINGS: SprayTheatreSettings = {
  stage_min_per_100k_denom_200: 3,
  stage_min_per_100k_denom_500: 2.5,
  stage_min_per_100k_denom_1000: 2,
  stage_min_per_100_usdc: 4,
  max_single_spray_ngn: 1_000_000,
  guest_session_cap_sec: 180,
  max_stage_sec: 2700,
  queue_compression_tiers: [
...
function benchmarkMinutesForDenom(
...
/** Linear interpolation between admin-configured queue tiers. */
export function queueCompressionMultiplier(
  queueDepth: number,
  tiers: QueueCompressionTier[],
): number {
  const sorted = [...tiers].sort((a, b) => a.min_queue - b.min_queue);
  if (sorted.length === 0) return 1;

  if (queueDepth <= sorted[0].min_queue) {
  ...
  for (let i = 1; i < sorted.length; i += 1) {
    ...
    const next = sorted[i];
    if (queueDepth <= next.min_queue) {
      const span = next.min_queue - prev.min_queue;
      if (span <= 0) return next.multiplier;
      const t = (queueDepth - prev.min_queue) / span;
      return prev.multiplier + t * (next.multiplier - prev.multiplier);
    }
  }
...
export function effectiveStageSec(
  baseStageSec: number,
  queueDepth: number,
  tiers: QueueCompressionTier[],
): number {
  return Math.round(baseStageSec * queueCompressionMultiplier(queueDepth, tiers));
}
...
export function computeSprayTheatrePlan(
  ...
  if (amount % denomination !== 0) {
    throw new Error("amount must be divisible by denomination");
  }
...
export function parseSprayTheatreSettings(raw: unknown): SprayTheatreSettings {
  if (!raw || typeof raw !== "object") return { ...DEFAULT_SPRAY_THEATRE_SETTINGS };
  const row = raw as Record<string, unknown>;
  const tiers = Array.isArray(row.queue_compression_tiers)
    ? (row.queue_compression_tiers as QueueCompressionTier[])
    : DEFAULT_SPRAY_THEATRE_SETTINGS.queue_compression_tiers;

  return {
    stage_min_per_100k_denom_200: Number(row.stage_min_per_100k_denom_200) || 3,
    stage_min_per_100k_denom_500: Number(row.stage_min_per_100k_denom_500) || 2.5,
    stage_min_per_100k_denom_1000: Number(row.stage_min_per_100k_denom_1000) || 2,
    stage_min_per_100_usdc: Number(row.stage_min_per_100_usdc) || 4,
    max_single_spray_ngn: Number(row.max_single_spray_ngn) || 1_000_000,
    guest_session_cap_sec: Number(row.guest_session_cap_sec) || 180,
    max_stage_sec: Number(row.max_stage_sec) || 2700,
